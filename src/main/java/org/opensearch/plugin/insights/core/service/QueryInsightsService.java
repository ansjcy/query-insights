...
public class QueryInsightsService extends AbstractLifecycleComponent {
    ...
    private void setExporterAndReader(final SinkType sinkType, final Map<String, IndexMetadata> indexMetadataMap) {
        ...
        final QueryInsightsReader reader = queryInsightsReaderFactory.createReader(TOP_QUERIES_LOCAL_INDEX_READER_ID, DEFAULT_TOP_N_QUERIES_INDEX_PATTERN, namedXContentRegistry);
        queryInsightsReaderFactory.updateReader(reader, DEFAULT_TOP_N_QUERIES_INDEX_PATTERN);

//        for (MetricType metricType : topQueriesServices.keySet()) {
//            TopQueriesService tqs = topQueriesServices.get(metricType);
//            tqs.setExporterType(sinkType, indexMetadataMap);
//            tqs.setReader(namedXContentRegistry);
//        }
    }
    ...
    private void deleteExpiredTopNIndices() {
        ...
        if (topQueriesExporter != null && topQueriesExporter.getClass() == LocalIndexExporter.class) {
            threadPool.executor(QUERY_INSIGHTS_EXECUTOR)
                .execute(() -> ((LocalIndexExporter) topQueriesExporter).deleteExpiredTopNIndices(clusterService.state().metadata().indices()));
        }
    }
     ...
     */
    private void deleteAllTopNIndices(final Client client, final Map<String, IndexMetadata> indexMetadataMap) {
